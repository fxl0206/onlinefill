		...
		try {
			...
			ResultSet rs1 = st
					.executeQuery("select taname,tatype from tablelist where taid='"
							+ tableid + "'");
			if (rs1.next()) {
				String tabname=rs1.getString("taname")+".doc";
				String tatype=".doc";
				String cotype="application/msword";
				cotype="text/html";	
				String ftype=rs1.getString("tatype");
				String sql="select * from `" + tableid+"` where acname='"+acname+"'";
				...
				{
				sql="select col from `"+tableid+"` where rowid='head'";
				rs.close();
